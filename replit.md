...
- **Incoming call banner**: When someone calls, an in-app banner slides down showing caller name, call type (voice/video), with Accept (green) and Decline (red) buttons. Auto-dismisses after 45s. Vibrates on arrival. Uses `incoming_call` socket event from server.
